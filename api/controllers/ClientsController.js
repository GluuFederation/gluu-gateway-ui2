...
module.exports = _.merge(_.cloneDeep(require("../base/Controller")), {
  ...
  opDiscovery: function (req, res) {
    ...
    const op_discovery_url =
      req.body.op_url + "/.well-known/openid-configuration";
    const option = {
      method: "POST",
    ...
    return new Promise(function (resolve, reject) {
      ...
      const option = {
        ...
        body: {
          ...
          redirect_uris: clientRequest.redirect_uris || [
          ...
          access_token_signing_alg:
            clientRequest.access_token_signing_alg || "RS256",
          allow_spontaneous_scopes:
            clientRequest.allow_spontaneous_scopes || false,
      ...
      sails.log(
        new Date(),
        ` $ curl -k -X POST ${
          clientRequest.oxd_url + "/register-site"
        } -d '${JSON.stringify(option.body)}'`
      );
  ...
  addGluuUMAPEP: function (req, res) {
    ...
    if (!body.uma_scope_expression) {
      return res
        .status(400)
        .send({ message: "uma_scope_expression is required" });
    }
  ...
  addOPClient: function (req, res) {
    ...
    return httpRequest(option)
      .then(function (response) {
        ...
        if (
          !(body.uma_scope_expression && body.uma_scope_expression.length > 0)
        ) {
        ...
        return httpRequest(option).then(function (response) {
          var clientToken = response.body;
          var option = {
            method: "POST",
            uri: body.oxd_url + "/uma-rs-protect",
      ...
      .then(function (response) {
        ...
        return sails.models.auditlog.create({
  ...
  updateGluuUMAPEP: function (req, res) {
    ...
    if (!body.oxd_id) {
      ...
      return res
        .status(500)
        .send({ message: "Provide oxd_id to update resources" });
    }
    ...
    if (!body.client_id) {
      ...
      return res
        .status(500)
        .send({ message: "Provide client_id to update resources" });
    }
    ...
    if (!body.client_secret) {
      ...
      return res
        .status(500)
        .send({ message: "Provide client_secret to update resources" });
    }
    ...
    return httpRequest(option)
      ...
      .then(function (response) {
        ...
        sails.log(
          ...
          ` $ curl -k -X POST ${
            body.oxd_url + "/uma-rs-protect"
          } -H 'Authorization: Bearer ${clientATToken}' -d '${JSON.stringify(
            option.body
          )}'`
        );

        return httpRequest(option).catch(function (e) {
      ...
      .then(function (response) {
        ...
        if (!result.oxd_id) {
          sails.log(
            new Date(),
            "Failed to update resources or update site",
            result
          );
          return Promise.reject({ message: "Failed to update resources" });
        }
  ...
  deleteGluuClientAuth: function (req, res) {
    ...
    sails.log(
      new Date(),
      ` $ curl -k -X POST ${
        sails.config.oxdWeb + "/get-client-token"
      } -d '${JSON.stringify(option.body)}'`
    );
    ...
    return httpRequest(option)
      .then(function (response) {
        ...
        if (body.oxd_id == sails.config.oxdId) {
          ...
          return Promise.reject({
            message: "Not allow to delete GG Admin login client",
          });
        }
        ...
        sails.log(
          new Date(),
          ` $ curl -k -X POST ${
            sails.config.oxdWeb + "/remove-site"
          } -H 'Authorization: Bearer ${
            clientToken.access_token
          }' -d '${JSON.stringify(option.body)}'`
        );
      ...
      .then(function (response) {
        ...
        if (!deletedClient.oxd_id) {
          sails.log(
            new Date(),
            "Failed to delete client from oxd",
            deletedClient
          );
          return Promise.reject({
            message: "Failed to delete client from oxd",
          });
        }
  ...
  deleteOPClientComment: function (req, res) {
    ...
    if (!body.route_id) {
      ...
      return res
        .status(500)
        .send({ message: "Provide route_id to add comment" });
    }
