...
module.exports = _.merge(_.cloneDeep(require("../base/Controller")), {
    ...
    sails.models.kongservices.find(
      ...
      function (err, extras) {
        ...
        extras.forEach(function (extra) {
          if (extra.tags instanceof Array) tags = tags.concat(extra.tags);
        });
  ...
  consumers: async (req, res) => {
    ...
    let plugins = await KongService.fetch(
      `/services/${serviceId}/plugins?enabled=true`,
      req
    );
    ...
    basicAuthPlugin = _.filter(
      plugins.data,
      (item) => item.name === "basic-auth"
    )[0];
    keyAuthPlugin = _.filter(
      plugins.data,
      (item) => item.name === "key-auth"
    )[0];
    hmacAuthPlugin = _.filter(
      plugins.data,
      (item) => item.name === "hmac-auth"
    )[0];
    oauth2Plugin = _.filter(plugins.data, (item) => item.name === "oauth2")[0];

    let aclConsumerIds;
    let authenticationPlugins = _.filter(
      plugins.data,
      (item) =>
        ["jwt", "basic-auth", "key-auth", "hmac-auth", "oauth2"].indexOf(
          item.name
        ) > -1
    );
    ...
    let whiteListedGroups = serviceAclPlugin
      ? serviceAclPlugin.config.whitelist || []
      : [];
    let blackListedGroups = serviceAclPlugin
      ? serviceAclPlugin.config.blacklist || []
      : [];
    ...
    let filteredAcls = _.filter(acls.data, (item) => {
      return (
        whiteListedGroups.indexOf(item.group) > -1 &&
        blackListedGroups.indexOf(item.group) === -1
      );
    });
    ...
    if (basicAuthPlugin)
      basicAuths = await KongService.fetch(`/basic-auths`, req);

    let jwtConsumerIds = jwts
      ? _.map(jwts.data, (item) => item.consumer.id)
      : [];
    let keyAuthConsumerIds = keyAuths
      ? _.map(keyAuths.data, (item) => item.consumer.id)
      : [];
    let hmacAuthConsumerIds = hmacAuths
      ? _.map(hmacAuths.data, (item) => item.consumer.id)
      : [];
    let oauth2ConsumerIds = oauth2
      ? _.map(oauth2.data, (item) => item.consumer.id)
      : [];
    let basicAuthConsumerIds = basicAuths
      ? _.map(basicAuths.data, (item) => item.consumer.id)
      : [];
    ...
    if (aclConsumerIds && aclConsumerIds.length) {
      sails.log(
        new Date(),
        "authenticationPluginsConsumerIds",
        authenticationPluginsConsumerIds
      );
      sails.log(new Date(), "aclConsumerIds", _.uniq(aclConsumerIds));
      consumerIds = authenticationPluginsConsumerIds.length
        ? _.intersection(
            _.uniq(aclConsumerIds),
            authenticationPluginsConsumerIds
          )
        : _.uniq(aclConsumerIds);
    } else {
    ...
    KongService.listAllCb(req, `/consumers`, (err, consumers) => {
      ...
      eligibleConsumers.forEach((consumer) => {
        ...
        if (
          keyAuths &&
          _.filter(keyAuths.data, (item) => item.consumer.id === consumer.id)
            .length
        ) {
        ...
        if (
          jwts &&
          _.filter(jwts.data, (item) => item.consumer.id === consumer.id).length
        ) {
          plugins.push("jwt");
        }
        if (
          hmacAuths &&
          _.filter(hmacAuths.data, (item) => item.consumer.id === consumer.id)
            .length
        ) {
        ...
        if (
          oauth2 &&
          _.filter(oauth2.data, (item) => item.consumer.id === consumer.id)
            .length
        ) {
          plugins.push("oauth2");
        }
        if (
          basicAuths &&
          _.filter(basicAuths.data, (item) => item.consumer.id === consumer.id)
            .length
        ) {
