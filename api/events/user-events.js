  ...
  createTransporter: function (cb) {
    ...
    sails.models.settings
      ...
      .exec(function (err, settings) {
        ...
        sails.log(
          new Date(),
          "helath_checks:createTransporter:settings =>",
          settings
        );
        ...
        sails.models.emailtransport
          ...
          .exec(function (err, transport) {
            ...
            sails.log(
              new Date(),
              "user-events:createTransporter:transport =>",
              transport
            );
            ...
            switch (settings[0].data.default_transport) {
              case "smtp":
                result.transporter = nodemailer.createTransport(
                  transport.settings
                );
                break;
              case "mailgun":
  ...
  notify: function (user) {
    ...
    self.createTransporter(function (err, result) {
      if (err || !result) {
        sails.log(
          new Date(),
          "user-events:failed to create transporter. No notification will be sent.",
          err
        );
      } else {
        ...
        if (!err) {
          var link =
            settings.baseUrl + "/auth/activate/" + user.activationToken ||
            "http://" +
              require("ip").address() +
              ":" +
              sails.config.port +
