...
module.exports = {
      ...
      sails.models.apihealthcheck
        ...
        .exec(function (err, hc) {
          if (err || !hc) {
            sails.log(
              new Date(),
              "api_health_checks => Failed to retrieve apiHealthCheck with id " +
                hc.id,
              err
            );
          } else {
            ...
            if (!hc.health_check_endpoint) {
              sails.log(
                new Date(),
                "api_health_checks =>",
                "no health_check_endpoint defined. Ending it."
              );
              return false;
            }
            ...
            sails.log(
              new Date(),
              "api_health_checks => Performing GET request to " +
                hc.health_check_endpoint
            );

            unirest.get(hc.health_check_endpoint).end(function (response) {
              if (response.error) {
                // health check failed
                if (!tasks[hc.id].firstFailed)
                  tasks[hc.id].firstFailed = new Date();
                ...
                sails.log(
                  new Date(),
                  "health_checks:cron:checkStatus => Health check for hc " +
                    hc.id +
                    " failed " +
                    tasks[hc.id].timesFailed +
                    " times"
                ...
                var timeDiff = sails.helpers.getMinutesDiff(
                  new Date(),
                  tasks[hc.id].lastNotified
                );
                sails.log(
                  new Date(),
                  "health_checks:cron:checkStatus:last notified => " +
                    tasks[hc.id].lastNotified
                );
                sails.log(
                  new Date(),
                  "health_checks:cron:checkStatus => Checking if eligible for notification",
                  timeDiff
                );
                if (
                  !tasks[hc.id].lastNotified ||
                  timeDiff > notificationsInterval
                ) {
                  self.notify(hc);
                }
              } else {
                ...
                sails.log(
                  new Date(),
                  "Health check for hc " + hc.id + " succeeded"
                );
                if (!tasks[hc.id].firstSucceeded)
                  tasks[hc.id].firstSucceeded = new Date();
                tasks[hc.id].timesFailed = 0;
    ...
    sails.models.apihealthcheck
      ...
      .exec(function (err, updated) {
        ...
        if (err) {
          sails.log(
            new Date(),
            "health_checks:updatehcHealthCheckDetails:failed",
            err
          );
        } else {
          sails.sockets.blast(
            "api.health_checks",
            _.merge({ hc_id: hcId }, data)
          );
        }
      });
  },
  ...
  createTransporter: function (settings, cb) {
    ...
    sails.models.emailtransport
      ...
      .exec(function (err, transport) {
        ...
        sails.log(
          new Date(),
          "health_checks:createTransporter:transport =>",
          transport
        );
        ...
        switch (settings.data.default_transport) {
          case "smtp":
            ...
            break;
          case "mailgun":
            result.transporter = hcmailer.createTransport(
              mg(transport.settings)
            );
            break;
        }
    ...
    sails.models.settings
      ...
      .exec(function (err, settings) {
        ...
        if (
          !settings.length ||
          !settings[0].data ||
          !settings[0].data.notify_when.api_down.active
        )
          return false;

        sails.helpers.sendSlackNotification(
          settings[0],
          self.makePlainTextNotification(hc)
        );
        ...
        self.createTransporter(settings[0], function (err, result) {
          if (err || !result) {
            sails.log(
              new Date(),
              "health_check:failed to create transporter. No notification will be sent.",
              err
            );
          } else {
            ...
            sails.helpers.getAdminEmailList(function (err, receivers) {
              sails.log(
                new Date(),
                "health_checks:notify:receivers => ",
                receivers
              );
              if (!err && receivers.length) {
                var mailOptions = {
                  from:
                    '"' +
                    settings.email_default_sender_name +
                    '" <' +
                ...
                if (settings.default_transport == "sendmail") {
                  sendmail(mailOptions, function (err, reply) {
                    if (err) {
                      sails.log.error(
                        new Date(),
                        "Health_checks:notify:error",
                        err
                      );
                    } else {
                      sails.log.info(
                        new Date(),
                        "Health_checks:notify:success",
                        reply
                      );
                    }
                  });
  ...
  notifyNotificationEndpoint: function (hc) {
    ...
    if (!hc.notification_endpoint) {
      sails.log(
        new Date(),
        "api_health_checks:notifyNotificationEndpoint => No notification endpoint defined"
      );
      return false;
    }

    unirest
      .post(hc.notification_endpoint)
      ...
      .end(function (response) {
        if (response.error) {
          sails.log(
            new Date(),
            "api_health_checks:notifyNotificationEndpoint => Failed to notify notification endpoint",
            response.error
          );
        } else {
          sails.log(
            new Date(),
            "api_health_checks:notifyNotificationEndpoint => Succeeded to notify notification endpoint"
          );
        }
      });
  },
  ...
  makeHTMLNotification: function makeHTMLNotification(hc) {
    var duration = moment
      .duration(moment().diff(moment(tasks[hc.id].lastSucceeded)))
      .humanize();

    var html =
      "<p>An API is down or unresponsive for more than " +
      duration +
      "</p>" +
  ...
  makePlainTextNotification: function (hc) {
    ...
    var text =
      ...
      " ] An API is down or unresponsive for more than " +
      duration +
      ". ID: " +
      hc.id +
      " | Name: " +
