...
module.exports = {
  ...
  schema: {
    ...
    info:
      "Konga uses the JWT method to authenticate with Google spreadsheets API." +
    ...
    form_fields: {
      connection: {
        ...
        ignore_first_row: {
          ...
          type: "boolean",
          description:
            "Whether or not to ignore the first row of the spreadsheet." +
      ...
      consumer: {
        username: {
          ...
          required: true,
          description:
            "The index of the spreadsheet column that will be used as the consumer <code>username</code>. The first column is 0.",
        },
        custom_id: {
          ...
          required: true,
          description:
            "The index of the spreadsheet column that will be used as the consumer <code>custom_id</code>. The first column is 0.",
        },
      },
    },
  },
    ...
    loadConsumers: function (req, res) {
      req.file("file").upload(function (err, uploadFiles) {
        ...
        var CREDENTIALS_PATH = path.join(
          __dirname,
          "..",
          "credentials/google-spreadsheets"
        );
        ...
        mkdirp(CREDENTIALS_PATH, function (err) {
          ...
          if (
            !uploadFiles.length &&
            !fs.existsSync(CREDENTIALS_PATH + "/credentials.json")
          ) {
            if (err)
              return res.notFound(
                "Credentials JSON file not found. You will need to upload one."
              );
          }
