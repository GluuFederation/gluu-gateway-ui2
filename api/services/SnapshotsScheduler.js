...
module.exports = {
  add: function (schedule) {
    if (!tasks[schedule.id]) {
      ...
      tasks[schedule.id] = cron.schedule(
        schedule.cron,
        function () {
          sails.log.debug(
            new Date(),
            "SnapshotScheduler:Running scheduled task",
            schedule
          );

          sails.models.kongnode
            .findOne(schedule.connection.id || schedule.connection)
      ...
      sails.log.debug(
        new Date(),
        "SnapshotScheduler:add",
        "A schedule with the same id already exists"
      );
    }
  },
      ...
      async.series(fns, function (err, data) {
        ...
        result.consumers.forEach(function (consumer) {
          consumerFns.push(function (cb) {
            KongService.listAllCb(
              node,
              "/consumers/" + consumer.id + "/acls",
              function (err, data) {
                if (err) {
                  return cb();
                }
          ...
          credentials.forEach(function (credential) {
            consumerFns.push(function (cb) {
              KongService.listAllCb(
                node,
                "/consumers/" + consumer.id + "/" + credential,
                function (err, data) {
                  if (err) {
                    return cb();
                  }
